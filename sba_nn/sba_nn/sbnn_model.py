...
#
# Create model with embeddings - with a layer 
# between the embeddings and the hidden layers,
# to get a NAICS representation that reflects
# the full hiearchy
#

def create_emb_model_intermediate(n_feat_base,
                                  naics_max_levels,
                                  naics_embedding_dim,
                                  naics_embedding_names = None,
                                  intermediate_dim = 8,
                                  intermediate_name = 'embedding_int',
                                  hidden_size = [128, 64],
                                  activation='tanh', 
                                  lr=0.0005,
                                  opt_func = tf.keras.optimizers.legacy.Adam, 
    ...
    ie = Dense(intermediate_dim, activation=activation, 
               name = intermediate_name)(concatenated_embeddings)
    ie = Dropout(dropout, name=f'dropout_{intermediate_name}')(ie)
    
    # Numeric inputs
    numeric_in_layer = Input(shape=(n_feat_base,), name='input_numeric')
        
    # Concatenate layers so far
    x = concatenate([ie] + [numeric_in_layer], name='input_concat')
